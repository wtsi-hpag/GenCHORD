...
\begin{document}
	...
	\section{Mathematical Theory}
		...
		\subsection{Attempt 4: Harmonic Network}
			...
			For this reason, we therefore find it much more convenient to think of the assignment of the harmonics to the genome as a \textit{network}, such that finding the optimal assignment is equivalent to finding the shortest path through the network.
			...
			\begin{figure}
				\begin{center}
					...
					\caption{An example of a harmonic network -- only two `jump vertices' are shown (in red and blue) for clarity. In the full network, every node $p_{iq}$ is connected to $p_{i+1,q}$ and $p_{i+L,k\neq q}$. The graph is directional - vertices can only be traversed from left to right.}
				\end{center}
			\end{figure}
