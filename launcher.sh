...
output=( "oes103/oes103-hic" "oes103/oes103-ccs" )
for i in "${!files[@]}"; do
	# echo ${files[i]} " goes to "  ${output[i]}
	echo "BEGINNING LAUNCHER ON " ${files[i]}
	./deforest -f ../Coverage/${files[i]}.dat -sigmaResolution 14 -worker 26 -sigmaMin 5 -sigmaMax 25 -o Output/${output[i]} -L 500000 -gamma 10 -thin 100 -accelerate 5 -Qmax 6 -alpha 0.1 
done
